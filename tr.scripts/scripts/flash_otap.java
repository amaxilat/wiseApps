...
//	String localControllerEndpointURL	= "http://" + InetAddress.getLocalHost().getCanonicalHostName() + ":8090/controller";
	...
	String nodeUrnsToReset 				= "urn:wisebed:ctitestbed:0x153d";
...
//	Future resetFuture = wsn.resetNodes(nodeURNs, 10, TimeUnit.SECONDS);
	List otapDevices = Lists.newArrayList() ;
	otapDevices.add(0x153d);
	File file = new File("/home/amaxilatis/repositories/wiseApps/iapps/ListenerApp/bin/JN5139R1/SecurityLampApplication.bin");
	InputStream is = new FileInputStream(file);
	long length = file.length();
    byte[] filebytes = new byte[(int)length];
	ISenseOtapAutomatedProgrammingRequest flashreq = new ISenseOtapAutomatedProgrammingRequest(otapDevices,filebytes);

	
	Future sendFuture = wsn.send(nodeURNs,HeaderAndJavaBeansXMLDecoderEncoder.encode(ISenseOtapAutomatedProgrammingRequest.SERIALIZATION_HEADER, flashreq), 10, TimeUnit.SECONDS);
