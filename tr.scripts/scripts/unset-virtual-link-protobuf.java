...
//--------------------------------------------------------------------------
	...
	Future future = wsn.destroyVirtualLink(vlinkFrom, vlinkTo,10,TimeUnit.SECONDS);
	JobResult jobResult = future.get();
	log.info("{}", jobResult);
	if (jobResult.getSuccessPercent() < 100) {
		System.out.println("The virtual link could not be unset. Exiting");
		System.exit(1);
	}


/*	Future future = wsn.setVirtualLink(vlinkTo, vlinkFrom, wsnEndpointURL, Lists.newArrayList(), Lists.newArrayList(), 5, TimeUnit.SECONDS);
	JobResult jobResult = future.get();
	log.info("{}", jobResult);
	if (jobResult.getSuccessPercent() < 100) {
