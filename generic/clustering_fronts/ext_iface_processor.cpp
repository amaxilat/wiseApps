                              ...
                              || message->destination() == BROADCAST_ADDRESS) )
         {
            ...
            if(!ext_delegates_.empty()){
                ...
                for ( ExtendedReceiverListIterator
                   ...
                   int id = (int)strtol(
                       message->source().label().substr((message->source().label()).rfind(":")+3, message->source().label().size()).data(), NULL, 16);

                      if(id >= BROADCAST_ADDRESS || id <= 0 )
                       id = 0;

                   (*it)(id, message->payload_size(), message->payload(), ex);
                }
            }
         }
         return true;
